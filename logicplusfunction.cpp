...
int main() {
	...
	cout << "Welcome"<<endl<<" The day has started and the batter is at 100%." << endl;
	...
	for (; package <= 'C'; package++) {
		...
		if (weather == 1) {
			...
			if (battery < 40) {
				...
				battery_drain = rand() % (25 - 10) + 10;
				battery = battery - battery_drain;
				cout << "The object "<<package<< " was delivered in windy weather. Remaining Battery : " << battery << endl;
			}
		}
		else if (weather == 3) {
			//Rainy Weather
			flight_status = "Delayed";
			cout << "Flight status due to rainy weather: " << flight_status;
			battery = battery - 0;
	...
	obstacle = rand() % (1 - 0) + 1;
	if (obstacle == 1) {
		battery_drain = rand() % (25 - 10) + 10; //Could be eliminated by placing inside weather if/else statement
